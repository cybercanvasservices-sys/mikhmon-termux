...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if (isset($_POST['name'])) {
    ...
    $record = '; :local mac $"mac-address"; :local time [/system clock get time ]; :local rawdate [/system clock get date]; :local date $rawdate; :if ([:find $rawdate "-"] != nil) do={:local year0 [:pick $rawdate 0 4]; :local month0 [:tonum [:pick $rawdate 5 7]]; :local day0 [:pick $rawdate 8 10]; :local months0 ("jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"); :set date ([ :pick $months0 ($month0 - 1)] . "/" . $day0 . "/" . $year0);}; :local year [:pick $date 7 11]; :local month [:pick $date 0 3]; /system script add name="$date-|-$time-|-$user-|-'.$price.'-|-$address-|-$mac-|-' . $validity . '-|-'.$name.'-|-$comment" owner="$month$year" source="$date" comment="mikhmon"';
    
    $onlogin = ':put (",'.$expmode.',' . $price . ',' . $validity . ','.$sprice.',,' . $getlock . ',"); {:local comment [ /ip hotspot user get [/ip hotspot user find where name="$user"] comment]; :local ucode [:pic $comment 0 2]; :if ($ucode = "vc" or $ucode = "up" or $comment = "") do={ :local rawdate [ /system clock get date ]; :local date $rawdate; :if ([:find $rawdate "-"] != nil) do={:local year0 [:pick $rawdate 0 4]; :local month0 [:tonum [:pick $rawdate 5 7]]; :local day0 [:pick $rawdate 8 10]; :local months0 ("jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"); :set date ([ :pick $months0 ($month0 - 1)] . "/" . $day0 . "/" . $year0);}; :local year [ :pick $date 7 11 ];:local month [ :pick $date 0 3 ]; /sys sch add name="$user" disable=no start-date=$rawdate interval="' . $validity . '"; :delay 5s; :local exp [ /sys sch get [ /sys sch find where name="$user" ] next-run]; :local getxp [len $exp]; :if ($getxp = 15) do={ :local d [:pic $exp 0 6]; :local t [:pic $exp 7 16]; :local s ("/"); :local exp ("$d$s$year $t"); /ip hotspot user set comment="$exp" [find where name="$user"];}; :if ($getxp = 8) do={ /ip hotspot user set comment="$date $exp" [find where name="$user"];}; :if ($getxp > 15) do={ /ip hotspot user set comment="$exp" [find where name="$user"];};:delay 5s; /sys sch remove [find where name="$user"]';
